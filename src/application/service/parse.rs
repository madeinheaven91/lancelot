...
pub fn parse_html_fl(html: Html) -> Vec<Task> {
    ...
    for task in &task_articles {
        ...
        let mut is_vacancy: bool;
        let binding= get_inner_html(task, ".b-post__foot b-post__foot_padtop_15");
        dbg!(&binding);
        match binding.contains("Заказ"){
            true => is_vacancy = false,
            _ => is_vacancy = true
        }


        let mut is_urgent = false;
        let selector = Selector::parse("img[href='https://st.fl.ru/images/urgently-1.png']").unwrap();
        let urgency: Vec<ElementRef<'_>> = task.select(&selector).collect();       // img href https://st.fl.ru/images/urgently-1.png        
        if !urgency.is_empty() { is_urgent = true; dbg!("Bebra"); };
        let is_pinned = false;
